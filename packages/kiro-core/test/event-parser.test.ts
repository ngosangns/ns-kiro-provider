...
describe("Feature 8: Stream Event Parsing", () => {
  ...
  describe("parseKiroEvent — metadataEvent token usage", () => {
    ...
    it.each([
      ...
      // Deliberate divergence from upstream: Kiro has never been observed to
      // send cache counters, so every spelling the underlying services use is
      // accepted rather than only the Smithy-modeled names.
      const e = parseKiroEvent("metadataEvent", {
        tokenUsage: { uncachedInputTokens: 1000, outputTokens: 20, ...cacheFields },
      });
      expect(e?.type === "usage" && e.data.cacheReadInputTokens).toBe(900);
      expect(e?.type === "usage" && e.data.cacheWriteInputTokens).toBe(120);
    });

    it("treats a zero cache read as reported, not absent", () => {
      const e = parseKiroEvent("metadataEvent", {
        tokenUsage: { uncachedInputTokens: 10, cacheReadInputTokens: 0 },
      });
      expect(e?.type === "usage" && e.data.cacheReadInputTokens).toBe(0);
    });

    it("ignores non-numeric and negative token counts", () => {
      const e = parseKiroEvent("metadataEvent", {
        tokenUsage: { uncachedInputTokens: "120", outputTokens: 7, cacheReadInputTokens: -1 },
      });
      expect(e?.type === "usage" && e.data.inputTokens).toBeUndefined();
      expect(e?.type === "usage" && e.data.outputTokens).toBe(7);
      expect(e?.type === "usage" && e.data.cacheReadInputTokens).toBeUndefined();
    });
  });
  ...
  describe("parseKiroEvent — meteringEvent is credits, not tokens", () => {
    ...
    it("parses the credit billing frame Kiro really emits", () => {
      ...
      const e = parseKiroEvent("meteringEvent", {
        unit: "credit",
        unitPlural: "credits",
        usage: 0.06586360271973467,
      });
      expect(e?.type === "metering" && e.data.credits).toBe(0.06586360271973467);
      expect(e?.type === "metering" && e.data.unit).toBe("credit");
    });

    it("omits a negative billed amount", () => {
      expect(parseKiroEvent("meteringEvent", { unit: "credit", usage: -1 })).toEqual({
        type: "metering",
        data: { unit: "credit" },
      });
    });
