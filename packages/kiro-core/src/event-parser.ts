...
// ABOUTME: Binary framing is handled by @smithy/core EventStreamMarshaller in response-stream.ts.
...
/**
 * Cache counters are read in every spelling the underlying services have been
 * seen to use — Kiro itself has not been observed to report any of them.
 */
...
function parseToolUse(parsed: Record<string, unknown>): KiroWireEvent | null {
...
function parseMetadata(parsed: Record<string, unknown>): KiroWireEvent | null {
  const tu = (parsed.tokenUsage ?? {}) as Record<string, unknown>;
  const data: KiroWireUsage = {
    inputTokens: tokenCount(tu.uncachedInputTokens) ?? tokenCount(tu.inputTokens),
    outputTokens: tokenCount(tu.outputTokens),
    totalTokens: tokenCount(tu.totalTokens),
    cacheReadInputTokens: firstTokenCount(tu, CACHE_READ_KEYS),
    cacheWriteInputTokens: firstTokenCount(tu, CACHE_WRITE_KEYS),
  ...
  for (const k of Object.keys(data) as (keyof KiroWireUsage)[]) {
 ...
 */
export function parseKiroEvent(key: string, parsed: Record<string, unknown>): KiroWireEvent | null {
  switch (key) {
    ...
    case "meteringEvent":
      ...
      return {
        type: "metering",
        data: { credits: tokenCount(parsed.usage), unit: str(parsed.unit), unitPlural: str(parsed.unitPlural) },
      };
 ...
 */
export function parseKiroEventByShape(parsed: Record<string, unknown>): KiroWireEvent | null {
