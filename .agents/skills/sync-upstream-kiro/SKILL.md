...
| **GitHub MCP** (`.mcp.json` → `.Codex/mcp/github-readonly.sh`) | Ngữ cảnh *vì sao* upstream đổi: mô tả PR, review comment, release note. Read-only, token lấy từ keyring `gh` lúc chạy |
...
```bash
node .Codex/skills/sync-upstream-kiro/scripts/upstream-status.mjs
```
...
Đổi contract nội bộ là được phép (xem workflow `execution` trong `~/.Codex/AGENTS.md`),
